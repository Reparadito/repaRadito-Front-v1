...
export const StartedSession = () => {
  const loggedGame  = useSelector((state) => state.videogamesState.isLogged);
  const token = useSelector((state) => state.videogamesState.gameToken);
  const dispatch = useDispatch();

  const handleUnloginGame = () => {
  ...
  <View>
    ...
    <View style={[styles.bgCont]}>
      <TouchableOpacity style={[styles.ImageButton]}>
        <Image
          source={loggedGame.image ?  {uri: loggedGame.image} : imageUser}
          style={{ margin: 5, width: 200, height: 200 , borderRadius:125}}
        />
      </TouchableOpacity>
      <TouchableOpacity style={[styles.miniButton]} onPress={handleUnloginGame}>
